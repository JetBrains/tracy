...
package org.jetbrains.ai.tracy.compiler.plugin
...
class TracyGeneratorExtension : IrGenerationExtension {
    private val traceAnnotationFqName = FqName("org.jetbrains.ai.tracy.core.instrumentation.Trace")

    @OptIn(UnsafeDuringIrConstructionAPI::class)
    override fun generate(moduleFragment: IrModuleFragment, pluginContext: IrPluginContext) {
        val withTraceSymbol = pluginContext.referenceFunctions(
            CallableId(FqName("org.jetbrains.ai.tracy.core.instrumentation.processor"), Name.identifier("withTrace"))
        ).findMultiplatformSymbol()
        val withTraceSuspendedSymbol = pluginContext.referenceFunctions(
            CallableId(FqName("org.jetbrains.ai.tracy.core.instrumentation.processor"), Name.identifier("withTraceSuspended"))
        ).findMultiplatformSymbol()
