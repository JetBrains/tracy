...
val langfuseJson = Json { ignoreUnknownKeys = true }
...
private suspend fun getTraceIds(projectId: String): List<String> { // TODO: decide how to understand how to take only needed traces
    val json = langfuseRequest(
        ...
        url = "${KotlinLangfuseClient.LANGFUSE_API}api/public/traces?limit=1"
    )
...
private suspend fun getSpansForTrace(traceId: String): List<JsonObject> {
    val trace = langfuseRequest(
        method = HttpMethod.Get,
        url = "${KotlinLangfuseClient.LANGFUSE_API}api/public/traces/$traceId"
    )
    ...
    val spans = spanIds.mapNotNull { obsId ->
        runCatching {
            langfuseRequest(
                method = HttpMethod.Get,
                url = "${KotlinLangfuseClient.LANGFUSE_API}api/public/observations/$obsId"
            )
        }.getOrNull()
    }
...
fun sortSpansByHierarchy(spans: List<JsonObject>): List<JsonObject> {
...
suspend fun createLangfuseProject(projectId: String, projectName: String): JsonObject { // TODO: add proper tokens
    ...
    val response = langfuseRequest(
        ...
        url = "${KotlinLangfuseClient.LANGFUSE_API}api/public/projects",
        body = payload
    )
    println(response)
    return response
...
suspend fun deleteLangfuseTrace(traceId: String): JsonObject {
    val response = langfuseRequest(
