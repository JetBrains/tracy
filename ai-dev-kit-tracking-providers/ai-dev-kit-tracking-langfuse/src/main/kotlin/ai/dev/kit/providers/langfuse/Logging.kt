        ...
        headers {
            append(
                HttpHeaders.Authorization, "Basic " + Base64.getEncoder().encodeToString(
                    "${KotlinLangfuseClient.config.publicKey}:${KotlinLangfuseClient.config.secretKey}".toByteArray()
                )
            )
        }
        body?.let { setBody(it) }
...
private suspend fun getTraceIds(projectId: String): List<String> { // TODO: decide how to understand how to take only needed traces
    val json = langfuseRequest(
        ...
        url = "${KotlinLangfuseClient.config.baseUrl}/api/public/traces?limit=1"
    )
...
private suspend fun getSpansForTrace(traceId: String): List<JsonObject> {
    val trace = langfuseRequest(
        method = HttpMethod.Get,
        url = "${KotlinLangfuseClient.config.baseUrl}/api/public/traces/$traceId"
    )
    ...
    val spans = spanIds.mapNotNull { obsId ->
        runCatching {
            langfuseRequest(
                method = HttpMethod.Get,
                url = "${KotlinLangfuseClient.config.baseUrl}/api/public/observations/$obsId"
            )
        }.getOrNull()
    }
...
private suspend fun createLangfuseProject(
    ...
    val response = langfuseRequest(
        ...
        url = "${KotlinLangfuseClient.config.baseUrl}/api/public/projects",
        body = payload
    )
...
suspend fun logScoreToLangfuse(
    ...
    langfuseRequest(
        method = HttpMethod.Post,
        url = "${KotlinLangfuseClient.config.baseUrl}/api/public/scores",
        body = payload
    )
}